...
function renderDashCajaBanner() {
  ...
  if (cajaActual) {
    ...
    const totalEgresosTurno = egresosDelTurno().reduce((s, e) => s + e.monto, 0);

    wrap.className = 'dash-caja-banner abierta';
    wrap.innerHTML = `
      <div class="dcb-info">
        <span class="dcb-estado ok">✅ Caja abierta</span>
        <span class="dcb-detalle">
          👤 ${cajaActual.cajero} · desde ${
            cajaActual.fechaApertura === now.toISOString().split('T')[0]
              ? 'hoy'
              : fechaLinda(cajaActual.fechaApertura)
          } ${cajaActual.horaApertura}
        </span>
        <span class="dcb-detalle" style="margin-top:2px">
          Apertura: ${fmt(cajaActual.montoApertura)} ·
          <span style="color:var(--green-light);font-weight:600">
            ${vsTurno.length} ventas · +${fmt(totalTurno)}
          </span>
        </span>
      </div>
      <div style="display:flex;flex-direction:column;align-items:flex-end;gap:8px">
        <span class="dcb-monto">${fmt(cajaActual.montoApertura + totalTurno - totalEgresosTurno)}</span>
        <div class="dcb-actions">
          <button class="btn btn-danger btn-sm" onclick="abrirModalCierre()">🔒 Cerrar caja</button>
          <button
            class="btn btn-sm"
            style="background:rgba(200,16,46,.18);color:#ff8888;border:1px solid rgba(200,16,46,.35)"
    ...
    wrap.innerHTML = `
      <div class="dcb-info">
        ...
        <span class="dcb-detalle">
          ${
            ultimo
              ? 'Último cierre: ' + fechaLinda(ultimo.fechaCierre) + ' ' + ultimo.horaCierre + ' · ' + ultimo.cajero
              : 'Sin historial de caja aún'
          }
        </span>
        ${
          ultimo
            ? `<span class="dcb-detalle" style="margin-top:2px">
                Contado: ${fmt(ultimo.efectivoContado)} · Diferencia:
                <span style="color:${
                  ultimo.diferencia === 0
                    ? '#52b788'
                    : ultimo.diferencia > 0
                  ...
                  ${ultimo.diferencia >= 0 ? '+' : ''}${fmt(ultimo.diferencia)}
                </span>
              </span>`
            : ''
        }
...
function renderDashChartSemana() {
  ...
  if (_dashChartSemana) {
    _dashChartSemana.destroy();
    _dashChartSemana = null;
  }
...
function renderDashChartHoras() {
  ...
  if (_dashChartHoras) {
    _dashChartHoras.destroy();
    _dashChartHoras = null;
  }
...
function renderDashInsumosCriticos() {
  ...
  const criticos = insumos
    ...
    .sort((a, b) => {
      const pa = a.stock / Math.max(a.minimo, 1);
      const pb = b.stock / Math.max(b.minimo, 1);
      return pa - pb;
    });
  ...
  if (!criticos.length) {
    cont.innerHTML = `
      <div style="display:flex;align-items:center;gap:10px;padding:10px 0">
        ...
        <span style="color:var(--green-light);font-size:13px;font-weight:600">
          Todos los insumos en nivel seguro
        </span>
      </div>
    `;
    return;
      ...
      <div style="padding:7px 0;border-bottom:1px solid rgba(255,255,255,.05)">
        <div style="display:flex;justify-content:space-between;margin-bottom:4px">
          ...
          <span style="font-size:12px;color:${color};font-weight:700">
            ${i.stock <= 0 ? 'AGOTADO' : i.stock + ' ' + i.unidad}
          </span>
        </div>
...
function renderDashUltimoCierre() {
  ...
  if (!cajaHistorial.length) {
    cont.innerHTML = `
      <div class="ins-empty" style="padding:20px 0">
        Sin cierres registrados aún ·
        <button class="btn btn-red btn-sm" onclick="abrirModalApertura()" style="margin-left:8px">
          Abrir primera caja
        </button>
      </div>
    `;
  ...
  const difColor =
    c.diferencia === 0 ? '#52b788' :
    c.diferencia > 0 ? '#8ab4f8' :
    '#ff6666';

  const difLabel =
    c.diferencia === 0 ? 'Cuadra ✅' :
  ...
  cont.innerHTML = `
    ...
    ${cajaHistorial.length > 1
      ? `<div style="margin-top:4px;font-size:12px;color:var(--muted);text-align:right">
          +${cajaHistorial.length - 1} cierre${cajaHistorial.length > 2 ? 's' : ''} anterior${cajaHistorial.length > 2 ? 'es' : ''}
          en historial
        </div>`
      : ''
    }
...
function renderEgresosTurno() {
  ...
  if (!lista.length) {
    cont.innerHTML = `
      <div style="color:var(--muted);font-size:13px;padding:12px 0;font-style:italic">
  ...
  cont.innerHTML = lista.map(e => {
    ...
    const prov = (window.proveedores || []).find(p => p.id == e.proveedorId);

    return `
      <div class="egreso-item">
        <span class="egreso-cat ${cat.cls}">${cat.emoji} ${cat.label}</span>

        <span class="egreso-desc">
          ${e.desc}
          ${e.subcat ? `<span style="color:var(--muted);font-size:11px"> · ${e.subcat}</span>` : ''}
          ${prov ? `<span style="color:var(--muted);font-size:11px"> · ${prov.nombre}</span>` : ''}
          <span style="color:var(--muted);font-size:11px"> · ${e.cajero}</span>
        </span>

        <span class="egreso-hora">${e.hora}</span>
        <span class="egreso-monto">-${fmt(e.monto)}</span>
        <span
          style="color:#ff6666;cursor:pointer;font-size:14px;opacity:.6"
    ...
    <div style="display:flex;justify-content:flex-end;padding:10px 16px 4px;font-size:13px;font-weight:700;color:#ff6666">
