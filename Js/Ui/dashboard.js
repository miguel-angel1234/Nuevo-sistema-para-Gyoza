...
function renderDashTopProds() {
  ...
  const map  = {};
  ventas.forEach(v => { v.items.forEach(i => { map[i.nombre] = (map[i.nombre] || { qty: 0, total: 0 }); map[i.nombre].qty += i.qty; map[i.nombre].total += i.precio * i.qty; }); });
  const top = Object.entries(map).sort((a, b) => b[1].qty - a[1].qty).slice(0, 5);
  if (!top.length) { cont.innerHTML = '<div style="color:var(--muted);font-size:13px;font-style:italic;padding:8px 0">Sin ventas aún</div>'; return; }
...
function renderDashChartHoras() {
  ...
  const horasMap = {};
  ventas.forEach(v => { const h = (v.hora || '00:00').split(':')[0] + ':00'; horasMap[h] = (horasMap[h] || 0) + v.total; });
...
function renderEgresosTurno() {
  ...
  cont.innerHTML = lista.map(e => {
    ...
    return `<div class="egreso-item">
      ...
      <span style="color:#ff6666;cursor:pointer;font-size:14px;opacity:.6" onclick="eliminarEgreso('${e.id}')" title="Eliminar">✕</span>
    </div>`;
  }).join('') + `<div style="display:flex;justify-content:flex-end;padding:10px 16px 4px;font-size:13px;font-weight:700;color:#ff6666">Total egresos: -${fmt(total)}</div>`;
}
